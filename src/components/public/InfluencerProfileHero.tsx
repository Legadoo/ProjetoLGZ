...
type InfluencerProfileHeroProps = {
  influencer: {
    name: string;
...
export function InfluencerProfileHero({ influencer }: InfluencerProfileHeroProps) {
  const displayName = influencer.nickname || influencer.name;
  ...
  return (
    <section className="lgz-container mt-6 overflow-hidden rounded-[2rem] border border-purple-500/30 bg-[radial-gradient(circle_at_75%_18%,rgba(176,38,255,0.34),transparent_32%),linear-gradient(135deg,#12001f,#030006)] shadow-[0_0_60px_rgba(126,34,206,0.18)]">
      <div className="grid gap-8 p-6 md:p-10 lg:grid-cols-[390px_1fr] xl:p-12">
        <div className="grid h-[420px] place-items-center overflow-hidden rounded-[1.7rem] border border-purple-500/40 bg-[radial-gradient(circle,rgba(176,38,255,0.28),transparent_55%),linear-gradient(135deg,#0b0014,#050008)] shadow-[0_0_40px_rgba(176,38,255,0.20)]">
          {influencer.profileImageUrl ? (
            <img src={influencer.profileImageUrl} alt={displayName} className="h-full w-full object-cover opacity-90" />
          ) : (
        ...
        <div className="flex flex-col justify-center">
          <div className="mb-6 flex flex-wrap gap-3">
            ...
            <span className="inline-flex items-center gap-2 rounded-full border border-purple-500/35 bg-black/35 px-5 py-3 text-xs font-black uppercase tracking-[0.22em] text-purple-200">
              <FaMicrophoneAlt />
              {influencer.contentCategory || "Criador Legendaryz"}
            </span>
          </div>
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-zinc-300">
            {influencer.fullBio || "Influenciador oficial da Legendaryz."}
          </p>
