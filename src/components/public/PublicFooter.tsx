    ...
    <footer className="mt-10 border-t border-purple-500/30 bg-black/70">
      <div className="lgz-container grid gap-8 py-10 md:grid-cols-[1.2fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-4 max-w-md text-sm leading-7 text-zinc-400">
            {config.footerText || "Organização de e-sports, comunidade gamer e plataforma digital. Seja lendário, faça parte da família LGZ."}
          </p>

          <div className="mt-6 flex gap-3 text-purple-300">
            {config.instagramUrl && (
              <a href={config.instagramUrl} target="_blank" className="grid h-11 w-11 place-items-center rounded-xl border border-purple-500/30 bg-purple-950/30 transition hover:bg-purple-700/40 hover:text-white" aria-label="Instagram Legendaryz">
                <FaInstagram />
              </a>
            )}

            {config.youtubeUrl && (
              <a href={config.youtubeUrl} target="_blank" className="grid h-11 w-11 place-items-center rounded-xl border border-purple-500/30 bg-purple-950/30 transition hover:bg-purple-700/40 hover:text-white" aria-label="YouTube Legendaryz">
                <FaYoutube />
              </a>
            )}

            {config.discordUrl && (
              <a href={config.discordUrl} target="_blank" className="grid h-11 w-11 place-items-center rounded-xl border border-purple-500/30 bg-purple-950/30 transition hover:bg-purple-700/40 hover:text-white" aria-label="Discord Legendaryz">
                <FaDiscord />
              </a>
            )}

            {config.tiktokUrl && (
              <a href={config.tiktokUrl} target="_blank" className="grid h-11 w-11 place-items-center rounded-xl border border-purple-500/30 bg-purple-950/30 transition hover:bg-purple-700/40 hover:text-white" aria-label="TikTok Legendaryz">
                <FaTiktok />
              </a>
            )}
          </div>
      ...
      <div className="border-t border-purple-500/20 py-5">
        <div className="lgz-container flex flex-col gap-3 text-sm text-zinc-500 md:flex-row md:items-center md:justify-between">
          <p>© 2026 {config.brandName}. Todos os direitos reservados.</p>
